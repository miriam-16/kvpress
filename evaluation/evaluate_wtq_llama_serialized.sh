dataset="wtq_tableqa_serialized"
...
for i in "${!press_names[@]}"; do
  ...
  (
    for max_capacity_context in "${max_capacity_contexts[@]}"; do
      ...
      python evaluate_script.py --dataset $dataset --model $model --press_name $press --max_capacity_context $max_capacity_context --device "cuda:0"
    done
  ) &
done
