...
for press_index in "${!press_names[@]}"; do
  ...
  for i in "${!data_dirs[@]}"; do
    ...
    # Run the experiment
    python evaluate_script.py --dataset "$dataset" --data_dir "$data_dir" --model "$model" --press_name "$press" --max_capacity_context "$max_capacity_context" --device "$gpu"
  done
done
